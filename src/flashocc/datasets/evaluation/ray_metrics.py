...
from flashocc.constants import (
    OCC_CLASS_NAMES, POINT_CLOUD_RANGE, VOXEL_SIZE,
)
...
def _get_dvr():
    ...
    return dvr
_pc_range = POINT_CLOUD_RANGE
_voxel_size = VOXEL_SIZE

occ_class_names = list(OCC_CLASS_NAMES)
...
def process_one_sample(sem_pred, lidar_rays, output_origin, instance_pred=None):
    ...
    for t in range(T): 
        ...
        with torch.no_grad():
            pred_dist, _, coord_index = _get_dvr().render_forward(
                occ_pred.cuda(),
...
def main_raypq(sem_pred_list, sem_gt_list, inst_pred_list, inst_gt_list, lidar_origin_list):
    ...
    for sem_pred, sem_gt, inst_pred, inst_gt, lidar_origins in \
        progress_bar(zip(sem_pred_list, sem_gt_list, inst_pred_list, inst_gt_list, lidar_origin_list), desc="RayPQ"):
...
def main(sem_pred_list, sem_gt_list, lidar_origin_list):
    ...
    for sem_pred, sem_gt, lidar_origins in progress_bar(zip(sem_pred_list, sem_gt_list, lidar_origin_list), desc="RayIoU"):
        sem_pred = torch.from_numpy(np.reshape(sem_pred, [200, 200, 16]))
        sem_gt = torch.from_numpy(np.reshape(sem_gt, [200, 200, 16]))
