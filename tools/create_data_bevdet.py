...
def add_ann_adj_info(extra_tag):
    ...
    for set in ["train", "val"]:
        ...
        for id in tqdm(range(len(dataset["infos"])), "add_ann_infos"):
        ...
        with open("%s/%s_infos_%s.pkl" % (dataroot, extra_tag, set), "wb") as fid:
            pickle.dump(dataset, fid, protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == "__main__":
    ...
    extra_tag = "flashocc2-nuscenes"
    nuscenes_data_prep(
        root_path=root_path, info_prefix=extra_tag, version=train_version, max_sweeps=0
    )
